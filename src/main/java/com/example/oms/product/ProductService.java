...
public class ProductService {
    ...
    public ProductResponseDTO createProduct(ProductRequestDTO productRequestDTO){
        List<Category> categoryList = categoryRepository.findAllById(productRequestDTO.getCategoryId());
        if(categoryList.size() != productRequestDTO.getCategoryId().size()){
    ...
    public ProductResponseDTO getProduct(Long id){
        Product product = productRepository.findById(id).orElseThrow(()->new EntityNotFoundException("User with id not found"));

        return modelMapper.map(product, ProductResponseDTO.class);
    }

    public Page<ProductResponseDTO> getAllProducts(String category, Pageable pageable){
